...
REPO_URL = "https://github.com/Emin-ahmedoff/GoldenUserBot"
LANG = LANG['MAIN']
...
async def botlog (String, Api, Hash):
    ...
    KanalId = await Client(CreateChannelRequest(
        title='GoldenUserBot BotLog',
        about=LANG['AUTO_BOTLOG'],
        megagroup=True
    ))
    KanalId = KanalId.chats[0].id

    Photo = await Client.upload_file(file='golden.png')
...
if __name__ == "__main__":
    ...
    if os.path.isdir("./GoldenUserBot/"):
        rm_r("./GoldenUserBot/")
    repo = Repo.clone_from(REPO_URL,"./GoldenUserBot/", branch="main")
    ...
    config['DEFAULT_BIO'] = "@GoldenUserBot"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/Emin-ahmedoff/GoldenUserBot"
    config['WARN_LIMIT'] = "3"
